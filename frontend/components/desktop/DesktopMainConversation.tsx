...
import type { WebSearchUsage } from '@/components/main/types';
import { toast } from 'sonner';
import { DesktopQuickActionCards, QUICK_ACTION_CARDS } from "./DesktopQuickActionCards";
import { QuickActionModal } from "@/components/main/QuickActionModal";

const MESSAGE_COLLAPSE_THRESHOLD = 300;
// changed: LOAD_OLDER_SCROLL_THRESHOLD removed â€” IntersectionObserver replaces the scroll threshold check
...
type MainConversationProps = {
  ...
  /** Number of messages queued while isLoading â€” shown as a badge on the stop button. */
...
export function DesktopMainConversation({
  activeSessionId,
  chatEndRef,
  chatError,
  chatScrollRef,
  ...
  useEffect(() => {
    ...
    const handleConversationScroll = () => {
      ...
      // changed: scroll-based trigger â€” reliable on all browsers including iOS Safari.
    ...
    chatScrollRef,
    handleLoadOlderMessages, // changed: safe to include â€” callback is now stable, won't cause churn
    hasOlderMessages,         // changed: safe to include â€” only changes when older messages are exhausted
    isLoadingChat,
    onScrollStateChange,
  ]);

  // changed: IntersectionObserver removed â€” replaced by scroll listener above (cross-browser reliable)
  ...
  }, [messages]); // changed: fires whenever the messages array changes â€” only acts when previousScrollHeight was captured
        ...
        <div className={`mx-auto px-4 sm:px-8 md:px-12 min-h-full flex flex-col ${!hasMessages ? 'w-full max-w-[1600px]' : 'max-w-[741px]'}`}>
          {isLoadingChat ? (
              ...
              <div className="hidden md:flex flex-1 w-full pt-[70px] pb-6">
                <div className="grid grid-cols-12 gap-12 lg:gap-16 xl:gap-20 w-full items-start">
                  ...
                  <div className="col-span-6 flex flex-col gap-4">
                    ...
                    <DesktopQuickActionCards onCardClick={setActiveCard} layoutMode="grid" />
                  </div>
                </div>
              </div>
                  ...
                  {activeCard === null && !inputMessage.trim() ? (
                    <>
                      ...
                      <AnimatePresence>
                        ...
                        >
                          <DesktopQuickActionCards onCardClick={setActiveCard} layoutMode="row" />
                        </motion.div>
                      </AnimatePresence>
                    </>
                  ) : null}
            ...
            <div className="py-4 flex flex-col">
              {/* changed: sentinel div removed â€” scroll listener replaces IntersectionObserver */}
