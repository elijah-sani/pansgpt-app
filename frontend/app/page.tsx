...
interface PDFDocument {
  ...
  course_code: string;    // Groouping Key
  lecturer_name: string;
  topic: string;
  drive_file_id: string;  // For Navigation
  file_name: string;
  file_size?: number;
}
...
export default function Home() {
  ...
  // ... (Fetch Docs and Auth Check remain the same)
  ...
  useEffect(() => {
    const getUser = async () => {
      ...
      if (currentUser?.email) {
        // Check DB for Admin Role (matches Admin Page logic)
          ...
          <div className="flex items-center gap-4">
            ...
            {authLoading ? (
              ...
              <div className="flex items-center gap-3 pl-3 border-l border-gray-200 dark:border-gray-800">
                ...
                {/* Admin Dashboard Button - Only if isAdmin is true */}
                {isAdmin && (
                  <Link
                    href="/admin"
