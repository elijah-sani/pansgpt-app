...
// ─── Desktop Auth Storage Persistence IPC Handlers ───────────────────────────
// [DESKTOP AUTH PERSISTENCE] Persists Supabase session data across random port launches
const getAuthStorePath = () => path.join(app.getPath("userData"), "auth-session.json"); // [DESKTOP AUTH PERSISTENCE]

const readAuthStore = () => { // [DESKTOP AUTH PERSISTENCE]
  try { // [DESKTOP AUTH PERSISTENCE]
    const filePath = getAuthStorePath(); // [DESKTOP AUTH PERSISTENCE]
    if (!fs.existsSync(filePath)) return {}; // [DESKTOP AUTH PERSISTENCE]
    const raw = fs.readFileSync(filePath, "utf-8"); // [DESKTOP AUTH PERSISTENCE]
    const parsed = JSON.parse(raw); // [DESKTOP AUTH PERSISTENCE]
    return parsed && typeof parsed === "object" ? parsed : {}; // [DESKTOP AUTH PERSISTENCE]
  } catch (err) { // [DESKTOP AUTH PERSISTENCE]
    console.warn("[DESKTOP AUTH PERSISTENCE] Failed to read auth-session.json:", err); // [DESKTOP AUTH PERSISTENCE]
    return {}; // [DESKTOP AUTH PERSISTENCE]
  } // [DESKTOP AUTH PERSISTENCE]
}; // [DESKTOP AUTH PERSISTENCE]

const writeAuthStore = (store) => { // [DESKTOP AUTH PERSISTENCE]
  try { // [DESKTOP AUTH PERSISTENCE]
    const filePath = getAuthStorePath(); // [DESKTOP AUTH PERSISTENCE]
    fs.writeFileSync(filePath, JSON.stringify(store, null, 2), "utf-8"); // [DESKTOP AUTH PERSISTENCE]
  } catch (err) { // [DESKTOP AUTH PERSISTENCE]
    console.error("[DESKTOP AUTH PERSISTENCE] Failed to write auth-session.json:", err); // [DESKTOP AUTH PERSISTENCE]
  } // [DESKTOP AUTH PERSISTENCE]
}; // [DESKTOP AUTH PERSISTENCE]

ipcMain.handle("auth:getItem", async (_event, key) => { // [DESKTOP AUTH PERSISTENCE]
  if (!key || typeof key !== "string") return null; // [DESKTOP AUTH PERSISTENCE]
  const store = readAuthStore(); // [DESKTOP AUTH PERSISTENCE]
  return store[key] ?? null; // [DESKTOP AUTH PERSISTENCE]
}); // [DESKTOP AUTH PERSISTENCE]

ipcMain.handle("auth:setItem", async (_event, key, value) => { // [DESKTOP AUTH PERSISTENCE]
  if (!key || typeof key !== "string") return; // [DESKTOP AUTH PERSISTENCE]
  const store = readAuthStore(); // [DESKTOP AUTH PERSISTENCE]
  store[key] = String(value); // [DESKTOP AUTH PERSISTENCE]
  writeAuthStore(store); // [DESKTOP AUTH PERSISTENCE]
}); // [DESKTOP AUTH PERSISTENCE]

ipcMain.handle("auth:removeItem", async (_event, key) => { // [DESKTOP AUTH PERSISTENCE]
  if (!key || typeof key !== "string") return; // [DESKTOP AUTH PERSISTENCE]
  const store = readAuthStore(); // [DESKTOP AUTH PERSISTENCE]
