...
if not origins:  # [GRACEFUL SHUTDOWN]
  ...
  # [GRACEFUL SHUTDOWN]
# [ELECTRON API CONFIG] SECURITY NOTE: Allowing any loopback port (127.0.0.1 / localhost) via regex is safe even with allow_credentials=True  # [ELECTRON API CONFIG]
# because loopback IPs are strictly reachable from processes on the user's local host machine. Remote malicious websites cannot forge  # [ELECTRON API CONFIG]
# or spoof an 'Origin: http://127.0.0.1:<port>' header in standard browsers, ensuring local Electron dynamic ports are safely authorized.  # [ELECTRON API CONFIG]
electron_and_preview_origin_regex = (  # [ELECTRON API CONFIG]
    r"^(https://pansgpt(?:-app)?-[a-zA-Z0-9]+-elijah-sanis-projects\.vercel\.app|"  # [ELECTRON API CONFIG]
    r"http://(127\.0\.0\.1|localhost):\d+)$"  # [ELECTRON API CONFIG]
)  # [ELECTRON API CONFIG]

app.add_middleware(  # [GRACEFUL SHUTDOWN]
    CORSMiddleware,  # [GRACEFUL SHUTDOWN]
    allow_origins=origins,  # [GRACEFUL SHUTDOWN]
    allow_origin_regex=electron_and_preview_origin_regex,  # [ELECTRON API CONFIG]
