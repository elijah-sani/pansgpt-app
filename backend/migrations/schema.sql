...
-- ─────────────────────────────────────────────────────────────
-- Learn Mode (add_learn_mode.sql)
-- ─────────────────────────────────────────────────────────────

-- Lazy-generated columns on document_sections
alter table public.document_sections
  add column if not exists explanation text,         -- [LEARN MODE] generated by TEXT_SECONDARY on first visit
  add column if not exists check_questions jsonb;    -- [LEARN MODE] [{question_text, options, correct_answer, explanation}]
...
create table if not exists public.document_learn_progress (
  ...
  last_score integer,
  updated_at timestamp with time zone not null default timezone('utc'::text, now()),
  constraint document_learn_progress_user_doc_section_unique
...
create index if not exists document_learn_progress_user_document_idx
  on public.document_learn_progress (user_id, document_id);

create index if not exists document_learn_progress_document_idx
...
create policy "document_learn_progress_select_policy"
on public.document_learn_progress for select
to authenticated
using (user_id = auth.uid() or public.is_super_admin());

create policy "document_learn_progress_insert_policy"
on public.document_learn_progress for insert
to authenticated
with check (user_id = auth.uid() or public.is_super_admin());

create policy "document_learn_progress_update_policy"
on public.document_learn_progress for update
to authenticated
using (user_id = auth.uid() or public.is_super_admin())
with check (user_id = auth.uid() or public.is_super_admin());

create policy "document_learn_progress_delete_policy"
on public.document_learn_progress for delete
to authenticated
using (user_id = auth.uid() or public.is_super_admin());

create policy "document_learn_progress_service_role_policy"
...
-- ─────────────────────────────────────────────────────────────
-- Learn Mode Retests (add_learn_retests.sql)                  -- [LEARN RETEST]
-- ─────────────────────────────────────────────────────────────

create table if not exists public.document_learn_pending_retests (  -- [LEARN RETEST]
