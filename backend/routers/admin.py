...
# [VECTOR REINDEX]
@router.get("/admin/vector-index-health", dependencies=[Depends(verify_api_key)])  # [VECTOR REINDEX]
async def get_vector_index_health(  # [VECTOR REINDEX]
    current_user: User = Depends(get_current_user),  # [VECTOR REINDEX]
):  # [VECTOR REINDEX]
    """  # [VECTOR REINDEX]
    Return vector index health status.  # [VECTOR REINDEX]
    Access: super_admin / global_admin only.  # [VECTOR REINDEX]
    """  # [VECTOR REINDEX]
    from dependencies import require_super_admin_role  # [VECTOR REINDEX]
    await require_super_admin_role(current_user)  # [VECTOR REINDEX]
  # [VECTOR REINDEX]
    sb = shared.supabase_service_client or shared.supabase_client  # [VECTOR REINDEX]
    if sb is None:  # [VECTOR REINDEX]
        raise HTTPException(status_code=503, detail="Database not available")  # [VECTOR REINDEX]
  # [VECTOR REINDEX]
    try:  # [VECTOR REINDEX]
        res = await asyncio.to_thread(  # [VECTOR REINDEX]
            lambda: sb.rpc("get_vector_index_health").execute()  # [VECTOR REINDEX]
        )  # [VECTOR REINDEX]
        if not res.data:  # [VECTOR REINDEX]
            raise HTTPException(status_code=500, detail="Failed to fetch vector index health data")  # [VECTOR REINDEX]
        return res.data[0] if isinstance(res.data, list) else res.data  # [VECTOR REINDEX]
    except Exception as exc:  # [VECTOR REINDEX]
        logger.error("[vector-index-health] Failed to get health: %s", exc)  # [VECTOR REINDEX]
        raise HTTPException(status_code=500, detail=f"Failed to fetch vector index health: {exc}")  # [VECTOR REINDEX]
  # [VECTOR REINDEX]
  # [VECTOR REINDEX]
# [VECTOR REINDEX]
@router.post("/admin/vector-index-reindex", dependencies=[Depends(verify_api_key)])  # [VECTOR REINDEX]
async def trigger_vector_index_reindex(  # [VECTOR REINDEX]
    current_user: User = Depends(get_current_user),  # [VECTOR REINDEX]
):  # [VECTOR REINDEX]
    """  # [VECTOR REINDEX]
    Trigger manual reindexing of the vector index concurrently.  # [VECTOR REINDEX]
    This calls trigger_vector_index_reindex() in the database, which uses pg_cron  # [VECTOR REINDEX]
    to run REINDEX INDEX CONCURRENTLY outside the transaction scope.  # [VECTOR REINDEX]
    Access: super_admin / global_admin only.  # [VECTOR REINDEX]
    """  # [VECTOR REINDEX]
    from dependencies import require_super_admin_role  # [VECTOR REINDEX]
    await require_super_admin_role(current_user)  # [VECTOR REINDEX]
  ...
  # [VECTOR REINDEX]
    try:  # [VECTOR REINDEX]
        res = await asyncio.to_thread(  # [VECTOR REINDEX]
            lambda: sb.rpc("trigger_vector_index_reindex").execute()  # [VECTOR REINDEX]
        )  # [VECTOR REINDEX]
        message = res.data if isinstance(res.data, str) else "Reindex job scheduled successfully"  # [VECTOR REINDEX]
        return {"status": "success", "message": message}  # [VECTOR REINDEX]
    except Exception as exc:  # [VECTOR REINDEX]
        logger.error("[vector-index-reindex] Failed to trigger reindex: %s", exc)  # [VECTOR REINDEX]
        raise HTTPException(status_code=500, detail=f"Failed to trigger reindex: {exc}")  # [VECTOR REINDEX]
